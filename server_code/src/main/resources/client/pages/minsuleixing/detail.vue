	...
	export default {
		...
		async onShow(options) {
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('minsuleixing', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','minsuleixing');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','minsuleixing');
			...
			async init(){
				...
				let res = await this.$api.info('minsuleixing', this.id);
				this.detail = res.data;
			},
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussminsuleixing/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
