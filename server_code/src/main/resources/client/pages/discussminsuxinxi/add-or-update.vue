	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discussminsuxinxi`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discussminsuxinxi`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discussminsuxinxi`, this.ruleForm);
						}else{
							await this.$api.add(`discussminsuxinxi`, this.ruleForm);
						}
						uni.setStorageSync('discussminsuxinxiCleanType',true);
