...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussminsuxinxiCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussminsuxinxiCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('discussminsuxinxi', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussminsuxinxi');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','discussminsuxinxi');
			...
			async init(){
				...
				let res = await this.$api.info('discussminsuxinxi', this.id);
				this.detail = res.data;
			},
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussminsuxinxi', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussdiscussminsuxinxi/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
