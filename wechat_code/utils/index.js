...
export function isAuth(tableName, key) {
    ...
    const menuMatch = menus.find(menu => menu.roleName === role);
    if (menuMatch) {
        const itemMatch = menuMatch.backMenu.some(menu => {
            return menu.child.some(child => child.tableName === tableName && child.buttons.includes(key));
        });
        return itemMatch;
    }
