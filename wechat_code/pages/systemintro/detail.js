...
const {
data
} = await detail("systemintro",id)
this.setData({
payAuth:utils.isAuthFront('systemintro','支付')
})

const detailList = data
this.setData({
detailList,
picture: detailList.picture1.split(','),
picture: detailList.picture2.split(','),
picture: detailList.picture3.split(','),
})
...
const resUpdate = await update('systemintro', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','systemintro');
...
async reply() {
const detailList = this.data.detailList
const res = await update("systemintro", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
