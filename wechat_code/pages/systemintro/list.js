...
this.setData({
addAuth: utils.isAuth("systemintro", "新增") || utils.isAuthFront("systemintro", "新增"),
delAuth: utils.isAuth("systemintro", "删除") || utils.isAuthFront("systemintro", "删除"),
editAuth: utils.isAuth("systemintro", "修改") || utils.isAuthFront("systemintro", "修改"),
})
if(options?.userid) {
this.setData({
...
page: this.data.currPage,
    limit: this.data.pageSize
            ...
            if(this.data.title){
            searchForm['title'] = '%' + this.data.title + '%'
        }
    

let res = {};
if(this.data.userid) {
res = await page(`systemintro`, searchForm);
} else {
res = await list(`systemintro`, searchForm);
}
...
wx.navigateTo({
url: `/pages/systemintro/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("systemintro",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
