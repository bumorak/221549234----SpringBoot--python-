...
const {
data
} = await detail("discussminsuxinxi",id)
this.setData({
payAuth:utils.isAuthFront('discussminsuxinxi','支付')
})
...
const commentRes = await list("discussdiscussminsuxinxi", commentData)
this.setData({
...
wx.setStorageSync('tableName',"discussminsuxinxi" )
wx.navigateTo({
url: `/pages/discussdiscussminsuxinxi/update-and-add`,
})
...
const resUpdate = await update('discussminsuxinxi', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
'cartForm.tablename': "discussminsuxinxi",
})
...
wx.setStorageSync('paytable','discussminsuxinxi');
...
async reply() {
const detailList = this.data.detailList
const res = await update("discussminsuxinxi", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
