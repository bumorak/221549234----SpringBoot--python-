...
const sessionReadArr=this.data.sessionReadArr
    ...
    this.data.cross ? (crossrefid = this.data.id, crossuserid = getApp().globalData.userInfo.id) : ""
        ...
        if(crossrefid && crossuserid) {
            ...
            let corssRes = await list(`discussminsuxinxi`, params)
            crossoptnum = wx.getStorageSync('statusColumnName');
...
if (ruleForm.id) {
await update(`discussminsuxinxi`, ruleForm)
}
else {
await add(`discussminsuxinxi`, ruleForm)
            }
        }
...
wx.setStorageSync('discussminsuxinxiCleanType', true);
