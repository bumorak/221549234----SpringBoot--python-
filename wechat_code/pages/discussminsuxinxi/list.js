...
this.setData({
addAuth: utils.isAuth("discussminsuxinxi", "新增") || utils.isAuthFront("discussminsuxinxi", "新增"),
delAuth: utils.isAuth("discussminsuxinxi", "删除") || utils.isAuthFront("discussminsuxinxi", "删除"),
editAuth: utils.isAuth("discussminsuxinxi", "修改") || utils.isAuthFront("discussminsuxinxi", "修改"),
})
if(options?.userid) {
this.setData({
...
let res = {};
if(this.data.userid) {
res = await page(`discussminsuxinxi`, searchForm);
} else {
res = await list(`discussminsuxinxi`, searchForm);
}
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("discussminsuxinxi",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/discussminsuxinxi/detail`
})
...
const {
    data
}= await page("discussminsuxinxi",obj)
...
const {
    data
}= await list("discussminsuxinxi",obj)
totalPage=data?.totalPage
