...
this.setData({
addAuth: utils.isAuth("minsuleixing", "新增") || utils.isAuthFront("minsuleixing", "新增"),
delAuth: utils.isAuth("minsuleixing", "删除") || utils.isAuthFront("minsuleixing", "删除"),
editAuth: utils.isAuth("minsuleixing", "修改") || utils.isAuthFront("minsuleixing", "修改"),
})
if(options?.userid) {
this.setData({
...
page: this.data.currPage,
    limit: this.data.pageSize
            ...
            if(this.data.minsuleixing){
            searchForm['minsuleixing'] = '%' + this.data.minsuleixing + '%'
        }
    

let res = {};
if(this.data.userid) {
res = await page(`minsuleixing`, searchForm);
} else {
res = await list(`minsuleixing`, searchForm);
}
...
wx.navigateTo({
url: `/pages/minsuleixing/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("minsuleixing",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
