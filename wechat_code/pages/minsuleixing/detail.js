...
const {
data
} = await detail("minsuleixing",id)
this.setData({
payAuth:utils.isAuthFront('minsuleixing','支付')
})
...
const resUpdate = await update('minsuleixing', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','minsuleixing');
...
async reply() {
const detailList = this.data.detailList
const res = await update("minsuleixing", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
