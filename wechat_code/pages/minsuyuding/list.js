...
this.setData({
addAuth: utils.isAuth("minsuyuding", "新增") || utils.isAuthFront("minsuyuding", "新增"),
delAuth: utils.isAuth("minsuyuding", "删除") || utils.isAuthFront("minsuyuding", "删除"),
editAuth: utils.isAuth("minsuyuding", "修改") || utils.isAuthFront("minsuyuding", "修改"),
})
if(options?.userid) {
this.setData({
...
let res = {};
if(this.data.userid) {
res = await page(`minsuyuding`, searchForm);
} else {
res = await list(`minsuyuding`, searchForm);
}
...
wx.navigateTo({
url: `/pages/minsuyuding/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("minsuyuding",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
