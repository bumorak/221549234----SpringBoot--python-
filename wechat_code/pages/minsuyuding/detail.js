...
const {
data
} = await detail("minsuyuding",id)
this.setData({
payAuth:utils.isAuthFront('minsuyuding','支付')
})
...
const resUpdate = await update('minsuyuding', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','minsuyuding');
...
async reply() {
const detailList = this.data.detailList
const res = await update("minsuyuding", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
